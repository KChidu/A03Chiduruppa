...
QUnit.test('Testing calculate currency', function (assert) {
    assert.equal(factorCalc('INR','USD'), 65, 'test with 2 different inputs');
    assert.equal(factorCalc('INR','INR'), 1, 'test with 2 same inputs');
    assert.equal(factorCalc('USD','INR'), 1/65, 'test with 2 different inputs');
    assert.equal(facCalc('EURO','USD'), 0.85, 'test with 2 different inputs');
    assert.equal(facCalc('EURO','EURO'), 1, 'test with 2 samw inputs');
    assert.equal(facCalc('USD','EURO'), 1/0.85, 'test with 2 different inputs');
});
